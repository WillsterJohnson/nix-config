...
new Cli('vault', {
	list: {
		...
		async action() {
			...
			for (const [uuid, { path }] of Object.entries(data.vaults)) table[path.split(sep).pop()!] = { uuid, path }
			console.table(table)
		},
	},
	open: {
		args: {
			vault: {
				type: 'string',
				defaultValue: null,
				description: 'Vault to open - a uuid (see "vault list"). Takes priority over "name"',
			},
			name: {
				type: 'string',
				defaultValue: null,
				description: 'Vault to open - a path (see "vault list")',
			},
			file: {
				type: 'string',
				defaultValue: null,
		...
		async action(_, named) {
			const paramMap = {
				vault:
					'vault' in named ? named.vault
					: 'name' in named ? join(vaultsPath, named.vault as string)
					: null,
				file: named.file,
			}
